...
Last run as:
    python construct_tiny-text-noise.py data/datasets/tiny-text-noise
"""
# encoding: utf-8
import argparse
import numpy as np
...
parser = argparse.ArgumentParser(description='Generate the tiny-text-noise dataset')
...
with open(f'{args.path}/meta', 'wb') as f:
    pickle.dump(dict(label_names=['alphabet'], vectors=np.zeros((1, 300))), f)
