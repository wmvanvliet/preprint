...
train_dataset = datasets.ImageFolder(
    root='/l/vanvlm1/large_word_stimuli/train',
    transform=transforms.Compose([transforms.ToTensor(), select_channel])
)

val_dataset = datasets.ImageFolder(
    root='/l/vanvlm1/large_word_stimuli/val',
    transform=transforms.Compose([transforms.ToTensor(), select_channel])
)
...
class ConvNet(torch.nn.Module):
    def __init__(self):
        ...
        self.features = torch.nn.Sequential(
            torch.nn.Conv2d(1, n_filters1, kernel_size=3, stride=1, padding=0),
            torch.nn.BatchNorm2d(n_filters1),
            torch.nn.ReLU(inplace=True),
            #torch.nn.MaxPool2d(kernel_size=2, stride=2),
            torch.nn.Conv2d(n_filters1, n_filters2, kernel_size=3, stride=1, padding=0),
            torch.nn.BatchNorm2d(n_filters2),
            torch.nn.ReLU(inplace=True),
            torch.nn.MaxPool2d(kernel_size=2, stride=2),
            torch.nn.Conv2d(n_filters2, n_filters3, kernel_size=3, stride=1, padding=0),
            torch.nn.BatchNorm2d(n_filters3),
            torch.nn.ReLU(inplace=True),
            torch.nn.MaxPool2d(kernel_size=2, stride=2))
        self.classifier = torch.nn.Linear(n_filters3 * 30 * 30, num_classes)
...
def text_phantom(text, size, fontsize=14, font='arial'):
    ...
    pil_font = ImageFont.truetype(
        font + ".ttf", size=fontsize // len(text), encoding="unic")
