...
def n(x):
    return x + (np.std(x) / 8) * np.random.randn(*x.shape)

print('Computing model DSMs...', end='', flush=True)
dsm_models = [
    mne_rsa.compute_dsm(n(feature_outputs[0]), metric='correlation'),
    ...
    mne_rsa.compute_dsm(n(semantic_outputs[0]), metric='correlation'),
