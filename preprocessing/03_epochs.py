...
all_epochs = []
for run in range(1, n_runs[subject] + 1):
    print('Loading run:', run)
    raw = mne.io.read_raw_fif(fname.raw_filt(subject=subject, run=run), preload=True)
    ...
    all_epochs.append(ica.apply(epochs))

all_epochs = mne.concatenate_epochs(all_epochs)
fig_evoked_clean = all_epochs.average().plot(show=False)
...
if subject == 1:
    ...
    metadata2.index = pd.Index(np.flatnonzero(all_epochs.events[:, 2] < 40))
    metadata2 = metadata2.reindex(index=pd.Index(np.arange(len(all_epochs))))
    metadata2.loc[np.flatnonzero(all_epochs.events[:, 2] == 40)] = metadata.query('question_asked')
    metadata2.loc[np.flatnonzero(all_epochs.events[:, 2] == 40), 'type'] = 'question'
    metadata2.loc[np.flatnonzero(all_epochs.events[:, 2] == 40), 'rotation'] = 0
    metadata2.loc[np.flatnonzero(all_epochs.events[:, 2] == 40), 'noise_level'] = 0
    metadata2.loc[np.flatnonzero(all_epochs.events[:, 2] == 40), 'text'] = metadata2.loc[np.flatnonzero(all_epochs.events[:, 2] == 40), 'question']
    metadata = metadata2
all_epochs.metadata = metadata
all_epochs.save(fname.epochs(subject=subject), overwrite=True)

# Save evokeds
evokeds = [all_epochs[cond].average() for cond in event_id.keys()]
