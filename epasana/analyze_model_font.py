...
def create_image_font(word='NAVETTA', size_=20,
                      rotation=0, fonts=None, noise_level=0,
                      data_path='/m/nbe/scratch/reading_models'):
    """Get the stimulus images presented during the MEG experiment with different text sizes
    Parameters
    ----------
    data_path : str
        Path to the epasana dataset on scratch.
    fonts: list | None
        if None, use the list below:
        fonts = ['ubuntu mono', 'courier', 'luxi mono regular',
                 'lucida console', 'lekton', 'dejavu sans mono',
                 'times new roman', 'arial', 'arial black', 'verdana',
                 'comic sans ms', 'georgia', 'liberation serif', 'impact',
                 'roboto condensed']
    ...
    images = []
    for font in fonts:
        img_np = create_image(word=word, rotation=rotation, fontsize=20,
                              font=font, noise_level=noise_level, 
                              data_path=data_path)
...
with open(os.devnull, "w") as f, contextlib.redirect_stdout(f):
    outputs = model.get_layer_activations(img_font,
                                          feature_layers=feature_layers,
                                          classifier_layers=classifier_layers)
    
# DSM of images with different fonts
DSM_pixelwise_font = mne_rsa.compute_dsm(img_font, metric='euclidean')
...
for i in range(n_layers_to_plot):
    ...
    rsa_font.append(mne_rsa.rsa(dsm, DSM_pixelwise_font))
    dsms_font.append(dsm)
    del dsm
...
for i in range(n_layers_to_plot-1, -1, -1): # loop through from the end to the first to reduce run time
    ax = axes.flat[i]
    sns.heatmap(distance.squareform(dsms_font[i]), ax=ax, square=True)
    del dsms_font[-1] # free memory

rsa_font_round = np.round(rsa_font, 2)    

feature = ("Feature Layer " + pd.Series(feature_layers, dtype=str)
           + " (RSA=" + pd.Series(rsa_font_round[:len(feature_layers)], dtype=str) + ")")
classifier = ("Classifier Layer " + pd.Series(classifier_layers, dtype=str)
             + " (RSA=" + pd.Series(rsa_font_round[len(feature_layers):len(feature_layers) + len(classifier_layers)], dtype=str) + ")")
if arch == 'vgg_sem':
    sem = "Semantics Layer (RSA=" + str(rsa_font_round[-1])  + ")"
...
# save model DSMs for imput with different fonts
fig.savefig(font_path)